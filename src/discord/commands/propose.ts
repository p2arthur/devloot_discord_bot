...
export class ProposeCommand {
  ...
  constructor(
    ...
    private threadService: DiscordThreadService,
  ...
  async handle(
    ...
    try {
      ...
      this.logger.warn(
        `GitHub API error for ${owner}/${repo}#${issueNumber}: ${status ?? 'unknown'}`,
      );
      if (status === 404) {
        await interaction.editReply(
    ...
    if (suggestionsChannelId) {
      try {
        ...
        if (channel && 'send' in channel) {
          ...
          await channelMsg.react('👎');
          await channelMsg.react('💵');
          threadCreated = await this.threadService.createProposalThread(
            channelMsg,
            issueTitle,
          );
        }
