...
export class DiscordGateway implements OnModuleInit {
  ...
  constructor(
    private readonly interactionService: DiscordInteractionService,
    private readonly messageService: DiscordMessageService,
    private readonly reactionService: DiscordReactionService,
    private readonly memberService: DiscordMemberService,
  ...
  async onModuleInit(): Promise<void> {
    ...
    try {
      ...
      this.logger.error(`Failed to register slash commands: ${this.describeError(err)}`);
      return;
    }

    this.client.on(Events.InteractionCreate, (interaction) => {
      void this.interactionService.handleInteraction(interaction, this.client);
    });
    this.client.on(Events.MessageCreate, (message) => {
      void this.messageService.handleMessage(message);
    });
    this.client.on(Events.MessageReactionAdd, (reaction, user) => {
      void this.reactionService.handleReactionAdd(reaction, user);
    });
    this.client.on(Events.GuildMemberAdd, (member) => {
      void this.memberService.handleNewMember(member);
    });
    ...
    this.client.on('clientReady', async () => {
      ...
      await this.messageService.setupVerifyChannel(this.client);
    ...
    try {
      ...
      this.logger.error(`client.login() FAILED: ${this.describeError(err)}`);
    }
  }
